...
def cumsum(iterable):
    ...
    wingStruct = iterable.next()
    yield wingStruct
    for c in iterable:
        wingStruct = wingStruct + c
        yield wingStruct
...
def rotate(numRotation,objectToRotate):
...
postZ2pSUP = [[26,3.6,0],[26.3,4.3,0],[0,1,0],[0,1,0]]

postUpZ2pSUP = [[24.8,4,0],[25.3,4.3,0],[-1,0,0],[-1,0,0]]

postZ3pSUP = [[26,3.6,8],[26.3,4.3,8],[0,1,0],[0,1,0]]
postZ3SUP = curveHermite(postZ3pSUP)

postUpZ3pSUP = [[24.8,4,8],[25.3,4.3,8],[-1,0,0],[-1,0,0]]
postUpZ3 = curveHermite(postUpZ3pSUP)

aboveP1 = [[26.3,4.3,0],[25.3,4.3,0],[0,-0.3,0],[0,0.3,0]]
aboveP2 = [[26.3,4.3,8],[25.3,4.3,8],[0,-0.3,0],[0,0.3,0]]


supWing = supHermite(postZ2pSUP,postUpZ2pSUP,[0,-0.3,0],[0,0.3,0])
supWing1 = supHermite(postZ3pSUP,postUpZ3pSUP,[0,-0.3,0],[0,0.3,0])
supWing2 = supHermite(postZ2pSUP,postZ3pSUP,[0,0,0],[0,0,0])
supWing3 = supHermite(postUpZ2pSUP,postUpZ3pSUP,[0,0,0],[0,0,0])
supWing4 = supHermite(aboveP1,aboveP2,[0,0,0],[0,0,0])

wingStruct = COLOR(grigio80)(STRUCT([supWing,postZ3SUP,postUpZ3,supWing1,supWing2,supWing3,supWing4]))
wing = T(2)(1)(T(1)(2)(R([1,3])(PI/2)(wingStruct)))

ventZpSup = [[17.5,4.4,0],[17.1,2.5,0],[1,-1,-3],[-2,-2,3]]


ventZp1Sup = [[17.5,4.4,0],[17.1,2.5,0],[9,0,0],[-2,-2,0]]

ventZpSup2 = [[17.5,4.4,0],[17.1,2.5,0],[1,-1,0],[-2,-2,0]]


supVent2 = supHermite(ventZpSup,ventZp1Sup,[0,0,0],[0,0,0])
supVent1 = supHermite(ventZpSup,ventZpSup2,[0,0,0],[0,0,0])
supVentS = COLOR(RED)(STRUCT([supVent2,supVent1]))
supVent = T(2)(1)(T(1)(-6.5)(R([1,3])(PI/2)(supVentS)))



#ESERCIZIO 4

steerTorus = creaToro([1,1.2])
steerTorusSmall = T(3)(-0.5)(creaToro([0.1,0.2]))
steerBlock = COLOR(YELLOW)(T(3)(-0.55)(CYLINDER([0.1,0.1])(24)))
steerBlockSmall = T(3)(-0.45)(COLOR(grigio80)(T(2)(0.05)(CYLINDER([0.03,0.004])(24))))
steerBolts = rotate(3, steerBlockSmall)

steerLine1p = [[1.1,0.1,0],[0.15,0.1,-0.5],[0.2,-0.2,-0.7],[-0.2,0.2,0.7]]
steerLine1 = curveHermite(steerLine1p)

steerLine2p = [[1.1,-0.1,0],[0.15,-0.1,-0.5],[0.2,-0.2,-0.7],[-0.2,0.2,0.7]]
steerLine2 = curveHermite(steerLine2p)

steerSup = supHermite(steerLine1p,steerLine2p,[0,0,0],[0,0,0])
steerRay = COLOR(BLACK)(steerSup)

circle = COLOR(YELLOW)(T(2)(1.099)(R([1,3])(PI/2)(PROD([annulusCircle(2*PI, 0.05, 0.11),Q(0.1)]))))
...
torus = creaToro([0.8,1.2])
rim = COLOR(grigio80)(T(3)(-0.1)(PROD([annulusCircle(2*PI, 0.75, 0.82),Q(0.2)])))
tb = COLOR(grigio80)(T(3)(-0.1)(PROD([annulusCircle(2*PI, 0.1, 0.2),Q(0.2)])))

tyreBoltHole = COLOR(BLACK)(T(3)(-0.1)(CYLINDER([0.1,0.2])(24)))
tyreBoltHoleSmall = T(2)(0.133)(COLOR(RED)(T(3)(-0.08)(CYLINDER([0.03,0.2])(24))))
tyreBolts = rotate(5, tyreBoltHoleSmall)

rayLine1p = [[-0.1,0.17,0.1],[-0.1,0.75,0.1],[0.2,-0.2,0.4],[-0.2,0.2,-0.4]]
rayLine1 = curveHermite(rayLine1p)

rayLine2p = [[0.1,0.17,0.1],[0.1,0.75,0.1],[-0.2,-0.2,0.4],[0.2,0.2,-0.4]]
rayLine2 = curveHermite(rayLine2p)

rayLine3p = [[-0.1,0.17,-0.1],[-0.1,0.75,-0.1],[0.2,-0.2,0.4],[-0.2,0.2,-0.4]]
rayLine3 = curveHermite(rayLine3p)

rayLine4p = [[0.1,0.17,-0.1],[0.1,0.75,-0.1],[-0.2,-0.2,0.4],[0.2,0.2,-0.4]]
rayLine4 = curveHermite(rayLine4p)

raySupRear = supHermite(rayLine3p,rayLine4p,[0,0,0],[0,0,0])
rayRear = COLOR(argento)(raySupRear)

raySupFront = supHermite(rayLine1p,rayLine2p,[0,0,0],[0,0,0])
rayFront = COLOR(argento)(raySupFront)

raySide1p = supHermite(rayLine1p,rayLine3p,[0,0,0],[0,0,0])
raySide1 = COLOR(BLACK)(raySide1p)

raySide2p = supHermite(rayLine2p,rayLine4p,[0,0,0],[0,0,0])
raySide2 = COLOR(BLACK)(raySide2p)

ray = STRUCT([rayFront,rayRear,raySide1,raySide2])
rays = rotate(6, ray)

wheel1 = S([1,2,3])([1.5,1.5,1.5])(T(1)(-4.5)(T(2)(1)(T(3)(4.3)(R([1,3])(PI/2)(STRUCT([torus,rim,tb,tyreBoltHole,tyreBolts,rays]))))))
wheel2 = T(3)(14)(wheel1)
...
ventZpSup = [[17.5,4.4],[17.1,2.5],[1,-1],[-2,-2]]
ventZ = curveHermite(ventZpSup)

ventZp1Sup = [[17.5,4.4],[17.1,2.5],[9,0],[-2,-2]]
ventZ1 = curveHermite(ventZp1Sup)
...
models = STRUCT([model,steer,wheel,wing,supVent])
